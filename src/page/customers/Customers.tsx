...
const Customers = () => {
  ...
  const handleEdit = (doctorId) => {
    ...
    navigate(`/customers/${doctorId}/edit`);
  };
  ...
  const handleToggleBlacklist = (customerId, isBlacklisted) => {
    fetch(
      `http://127.0.0.1:8000/api/customers/${customerId}/toggle-blacklist`,
      {
        method: "POST",
        headers: {
        ...
        setCustomers((prevCustomers) =>
          prevCustomers.map((customer) =>
            customer.id === customerId
              ? { ...customer, is_blacklisted: !isBlacklisted }
              : customer
          )
        );
      })
    ...
    },
    {
      ...
      renderCell: (params) => (
        <>
          ...
          <Button
            variant="outlined"
            color={params.row.is_blacklisted ? "error" : "warning"}
            sx={{ margin: 1 }}
            onClick={() =>
              handleToggleBlacklist(params.row.id, params.row.is_blacklisted)
            }
          >
            {params.row.is_blacklisted
            ...
            <Table sx={{ minWidth: 650 }} aria-label="Reservation Details">
              <TableBody>
                ...
                <StyledTableRow>
                  ...
                  <StyledTableCell>
                    {selectedCustomer?.nationality?.name || "N/A"}
                  </StyledTableCell>
                  <StyledTableCell sx={{ fontWeight: 650 }}>
                    email
                  </StyledTableCell>
