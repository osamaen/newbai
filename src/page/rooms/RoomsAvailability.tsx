...
const RoomsAvailability = () => {
  ...
  const [openBedSpaceModal, setOpenBedSpaceModal] = useState(false);
  ...
  const handleBedSpaceClick = (row) => {
    // setSelectedRoomId(row); // Set selected room ID
    setOpenBedSpaceModal(true);
    setBedSpaces(row.bed_spaces);
    console.log(row);
      ...
      renderCell: (params) => (
        <>
          <Button
            ...
            onClick={() => handleBedSpaceClick(params.row)}
          >
            Add Reservation
          </Button>
        </>
  ...
  return (
    <Box>
      ...
      <Modal
        open={openBedSpaceModal}
        onClose={() => setOpenBedSpaceModal(false)}
